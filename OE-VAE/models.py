...
class Encoder(layers.Layer):
    """ Mapping inputs to (z_mean, z_log_var, z) """
    def __init__(self, layers, latent_dim=10, intermediate_dim=40, seed=None, name="encoder", **kwargs):
        super(Encoder, self).__init__(name=name, **kwargs)
        self.seed          = seed
    ...
    def call(self, inputs):
        ...
        z_mean    = self.dense_mean(x)
        z_log_var = self.dense_log_var(x)
        z         = self.sampling([z_mean, z_log_var], self.seed)
        return z_mean, z_log_var, z


class Decoder(layers.Layer):
    """ Converting the encoded digit vector z back to input space """
    def __init__(self, inputs_dim, layers, intermediate_dim=64, name="decoder", **kwargs):
        super(Decoder, self).__init__(name=name, **kwargs)
        self.dense_proj   = layers.Dense(intermediate_dim, activation="relu")
        self.dense_output = layers.Dense(inputs_dim, activation="sigmoid")
    def call(self, inputs):
        x = self.dense_proj(inputs)
        return self.dense_output(x)
...
class VariationalAutoEncoder(models.Model):
    """ Combining the encoder and decoder into a VAE model for training """
    def __init__(self, inputs_dim, layers, intermediate_dim=64, latent_dim=32, seed=None, name="autoencoder", **kwargs):
        super(VariationalAutoEncoder, self).__init__(name=name, **kwargs)
        self.inputs_dim = inputs_dim
        self.encoder    = Encoder(layers, latent_dim=latent_dim, intermediate_dim=intermediate_dim, seed=seed)
        self.decoder    = Decoder(inputs_dim, layers, intermediate_dim=intermediate_dim)
    def call(self, inputs):
        z_mean, z_log_var, z = self.encoder(inputs)
        reconstructed = self.decoder(z)
        # Adding KLD regularization loss
