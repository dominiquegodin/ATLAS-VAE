...
parser.add_argument( '--scaler_out'   , default = ''                                  )
...
if args.scaler_type.lower() != 'none' and os.path.isfile(args.scaler_in):
    print('\nLoading scaler/transformer from: ' + args.scaler_in)
...
if args.n_epochs > 0:
    ...
    OoD_sample['constituents'] = apply_scaler(OoD_sample['constituents'], args.n_dims, scaler, 'OoD')
...
if args.plotting == 'ON':
    metric_list = ['Latent','MAE'] #+ ['Inputs'] #+ ['X-S','JSD','EMD','KSD','KLD']
    loss_metric = 'Latent' if (args.OE_type=='KLD' and 'Latent' in metric_list) else 'MAE'
