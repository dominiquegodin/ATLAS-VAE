...
from scipy.special import gammainc, gammaincc #as G  # Need G(a,b) for the gamma function
...
class BumpHunter1D:
    ...
    def _scan_hist(self, hist, ref, w_ar, ih: int):
        ...
        for i, w in enumerate(w_ar):
            ...
            # Adding robustness with infinte precision gamma functions #DG
            def get_res(Nhist, Href, idx):
                if   Nhist[idx] > Href[idx] and Href[idx] > 0:
                    res = mpmath.gammainc (Nhist[idx]  , 0        , Nref[idx], regularized=True)
                elif Nhist[idx] < Nref[idx]:
                    res = mpmath.gammaincc(Nhist[idx]+1, Nref[idx], np.inf   , regularized=True)
                return res
            cuts = (((Nhist>Nref) & (Nref>0)) | (Nhist<Nref)) & (np.abs(res[i])==np.inf)
            #if np.sum(cuts) !=0: print(np.sum(cuts))
    ...
    def _scan_hist_multi(self, hist, ref, w_ar, ih: int):
        ...
        for ch in range(len(hist)):
            ...
            for i, w in enumerate(w_ar):
                ...
                if self.mode == "excess":
                    res[(Nhist > Nref) & (Nref > 0)] = gammainc(
                        Nhist[(Nhist > Nref) & (Nref > 0)],
                        Nref[(Nhist > Nref) & (Nref > 0)],
                    )
                elif self.mode == "deficit":
                    res[Nhist < Nref] = 1.0 - gammainc(
                        Nhist[Nhist < Nref] + 1, Nref[Nhist < Nref]
                    )
    ...
    def bump_scan(
        ...
        if self.t_ar.size > 1:
            ...
            if self.global_Pval == 1:
                ...
                self.significance = -norm.ppf(1 / self.npe) #DG
                if verbose:
    ...
    def signal_inject(self, sig, bkg, is_hist: bool = False):
        ...
        and (self.global_Pval > 1 / self.npe):
            ...
            if global_inf <  1 / self.npe:
                ...
                sigma_inf = -norm.ppf(1 / self.npe) #DG
            else:
            ...
            if global_sup <  1 / self.npe:
                ...
                sigma_sup = -norm.ppf(1 / self.npe) #DG
            else:
    ...
    def plot_bump(
        ...
        def get_sig(H, Hbkg, idx):
            if   H[idx] > Hbkg[idx] and Hbkg[idx] > 0:
                ...
                sig = mpmath.gammaincc(H[idx]+1, Hbkg[idx], np.inf   , regularized=True)
            return np.sqrt(2)*mpmath.erfinv(1-2*sig)
        cuts = (((H[0]>Hbkg) & (Hbkg>0)) | (H[0]<Hbkg)) & (np.abs(sig)==np.inf)
        sig[cuts] = [get_sig(H[0][cuts], Hbkg[cuts], idx) for idx in range(np.sum(cuts))]
        ...
        if make_histo:
            ...
            plt.vlines(
                ...
                0,
                #H[0].max(),
                [histo[0][np.argmin(np.abs(Bmin-H[1]))], histo[0][np.argmin(np.abs(Bmax-H[1]))]], #DG
                colors="r",
                linestyles='dashed',
                label="BUMP"
                )
    ...
    def bump_info(self, data, is_hist: bool=False, verbose: bool=True):
        """
        ...
        if multi_chan:
            ...
            bstr += f'Local significance (combined) : {-norm.ppf(self.min_Pval_ar[0].prod()):.5g}\n' #DG
        else:
            # Append results for only one channel (no 'combined')
            bstr += f'\nBump edges : [{Bmin:.3g}, {Bmax:.3g}]'
    ...
    def print_bump_info(self):
        ...
        if not isinstance(self.min_Pval_ar[0], np.ndarray):
            ...
            print(f"   local significance = {-norm.ppf(self.min_Pval_ar[0]):.5f}") #DG
        else:
            ...
            print(f"   local significance (combined) = {-norm.ppf(self.min_Pval_ar[0].prod()):.5f}") #DG
