...
class BumpHunter1D:
    ...
    def _scan_hist(self, hist, ref, w_ar, ih: int):
        ...
        for i, w in enumerate(w_ar):
            ...
            def get_res(Nhist, Nref, idx):
                if   Nhist[idx] > Nref[idx] and Nref[idx] > 0:
                    result = mpmath.gammainc(Nhist[idx]  , 0        , Nref[idx], regularized=True)
                elif Nhist[idx] < Nref[idx]:
                    result = mpmath.gammainc(Nhist[idx]+1, Nref[idx], np.inf   , regularized=True)
                return result
            cuts = (((Nhist>Nref) & (Nref>0)) | (Nhist<Nref)) & ((np.abs(res[i])==np.inf) | (np.abs(res[i])<=1e-300))
    ...
    def bump_scan(
        ...
        if not multi_chan:
            self.min_Pval_ar = self.min_Pval_ar[self.min_Pval_ar > 0] #DG
            #with mpmath.workdps(10):
