...
def plot_results(y_true, X_true, X_pred, sample, train_var, n_dims, metrics, model, encoder, output_dir):
    ...
    if len(set(train_var)-{'constituents'}) == 0:
        X_true_dict['B-1'] = sample['constituents']
    elif 'constituents' not in train_var:
        X_true_dict['B-1'] = sample['scalars']
    else:
        X_true_dict['B-1'] = np.concatenate([sample['constituents'], sample['scalars']], axis=1)
    for key in ['DNN', 'FCN']:
...
def var_distributions(samples, output_dir, sig_bins, bkg_bins, var, normalize=False, density=True, log=True):
    ...
    for n in [0, 1]:
        for sample in samples:
            ...
            # weights histograms
            #pylab.hist(weights, bins, histtype='step', weights=None, color=colors[n], lw=2, log=log,
            #           alpha=alphas[samples.index(sample)], label=labels[samples.index(sample)][n])
    if normalize:
        if log:
        ...
        #pylab.xlim(0, 300); pylab.ylim(1e-1, 1e7)
        pylab.xlim(0, 6000)#; pylab.ylim(1e1, 1e6)
...
def loss_distributions(y_true, X_loss, weights, metric, output_dir, n_bins=100):
    ...
    max_dict  = {'JSD':0.4, 'MSE':0.00005, 'MAE':0.3   , 'EMD':25   , 'KLD':0.3,
...
def mass_correlation(y_true, X_losses, X_mass, metrics_list, output_dir, n_bins=100):
    colors_dict  = {'JSD':'tab:blue'  , 'MSE':'tab:green', 'EMD'   :'tab:orange' , 'KLD':'tab:red',
                    'X-S':'tab:purple', 'MAE':'tab:brown', 'Latent':'tab:cyan'   ,
                    'B-1':'gray'      , 'B-2':'darkgray' , 'DNN'   :'tab:pink'   , 'FCN':'tab:orange'}
    max_dict  = {'JSD':0.4, 'MSE':0.00005, 'MAE':0.3   , 'EMD':25 , 'KLD':0.3,
                 'X-S':0.3, 'B-1':0.1 , 'B-2':0.0175, 'DNN':100, 'FCN':100, 'Latent':0.5}

    plt.figure(figsize=(12,8)); pylab.grid(True); axes = plt.gca()
    axes.xaxis.set_minor_locator(ticker.AutoMinorLocator(10))
    axes.tick_params(axis='both', which='major', labelsize=14)
    plt.xlabel('Signal Efficiency', fontsize=24)
    plt.ylabel('JSD (before/after cut)', fontsize=24)
    labels = [r'$t\bar{t}$ jets', 'QCD jets']; colors = ['tab:orange', 'tab:blue']
    for metric in metrics_list:
        X_loss = X_losses[metric]
        max_loss = min(max_dict[metric], np.max(X_loss))

        for n in [0]:#set(y_true):
            ...
            JSD          = []
            for loss in losses:
                efficiencies += [ np.sum(X_loss>loss)/len(X_loss) ]
                X_mass_cut = X_mass.copy()
                X_mass_cut[X_loss<=loss] = np.random.choice(X_mass_cut[X_loss<=loss], np.sum(X_loss<=loss))
                JSD += [ distance.jensenshannon(X_mass, X_mass_cut, base=2) ]

            plt.plot(efficiencies, JSD, label=str(metric), color=colors_dict[metric], lw=2)
    pylab.xlim(0, 1)
    plt.legend(loc='upper right', fontsize=18, ncol=2 if len(metrics_list)<9 else 3)
    file_name = output_dir+'/'+'metrics'+'_correlation.png'
