...
def make_sample(bkg_idx, sig_idx, n_dims, metrics, train_var, n_constituents, bkg_cuts, sig_cuts, bkg, sig):
    ...
    if sig_idx[0] != sig_idx[1]:
        sig_sample = load_data(sig_file, sig, sig_idx, metrics, train_var, n_constituents, cuts=sig_cuts)
    if bkg_idx[0] != bkg_idx[1]:
        bkg_sample = load_data(bkg_file, bkg, bkg_idx, metrics, train_var, n_constituents, cuts=bkg_cuts)
    ...
    if 'constituents' in sample and True: #pt_scaling
...
def load_data(data_file, sample_type, idx, metrics, train_var, n_constituents, cuts=''):
    ...
    sample   = {key:data[key][idx[0]:idx[1]] for key in data_var if key!='constituents'}
    if 'constituents' in data_var:
        sample['constituents'] = data['constituents'][idx[0]:idx[1],:4*n_constituents]
    ...
    if sample_type == 'W': sample.update({key:val for key,val in jets_4v(sample['constituents']).items()})
    ...
    if len(set(train_var)-{'constituents'}) != 0:
        sample['scalars'] = np.hstack([sample.pop(key)[:,np.newaxis] for key in train_var if key!='constituents'])
...
def fit_scaler(sample, n_dims, scaler_out, reshape=True, scaler_type='QuantileTransformer'):
...
def s_scaling(sample, scaler):
    print('Applying scalars scaling', end=' ', flush=True); start_time = time.time()
    print('(', '\b'+format(time.time() - start_time, '2.1f'), '\b'+' s)')
    sample = scaler.transform(sample)
    return sample
def apply_scaler(sample, n_dims, scaler, idx=(0,None), return_dict=None, reshape=True):
    if idx == (0,None):
        print('Applying constituents scaling', end=' ', flush=True); start_time = time.time()
    shape = sample.shape
    if reshape: sample = np.reshape(sample, (-1,n_dims))
    sample = scaler.transform(sample)
